# Artefak Bukti — database `penandaan`

Hasil eksekusi nyata terhadap database live lewat tunnel `postgresql://localhost:5533/penandaan`,
...
| Berkas | Isi | Bentuk |
|---|---|---|
| `pair_ringkas.csv` | **43 pertanyaan**, satu baris fisik per pertanyaan. Kolom: tabel yang disentuh, pakai agregasi atau tidak, status sebelum/sesudah terjemahan, cacah baris hasil, **kode diagnosa**, dan penjelasan sebabnya | CSV rapi — aman dibuka di Excel / `pandas.read_csv` |
...
| `csv_sql_training.csv` | **8 SQL** kolom *SQL Training* dari `BPOM User Relevant Query.xlsx` untuk modul ini + hasil eksekusinya | CSV rapi |
...
| Set | Hasil |
|---|---|
| Pair `context_stores` | **40 dari 43 menghasilkan data (93%)** |
| `SQL Training` dari CSV | **7 dari 8** |
