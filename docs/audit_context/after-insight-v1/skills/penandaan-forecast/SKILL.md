---
name: penandaan-forecast
description: "Forecasting skill for predicting future penandaan volume. Computes projections deterministically from historical data with quality labels."
tags: [bpom, penandaan, forecast]
version: "1.0.0"
---

# Penandaan Forecaster (Trigger)

Routes BPOM penandaan forecast requests to `run_forecast`. This skill owns only the
penandaan CAPTURE parameters — all compute is deterministic (ETS seasonal fit
...
- Default series = Total Penandaan (all komoditi, all balai).
...
- Build the SQL using `forecast_guide.md` §1's template exactly (table +
  filter from §3's registry). If the **series** is ambiguous, call
...
|---|---|---|
| "penandaan **baru**/per bulan" | flow | `SUM(jumlah_penandaan)` |
| "penandaan **total**/**akumulasi**" | stock | `SUM(SUM(jumlah_penandaan)) OVER (ORDER BY tanggal_periode)` |
...
- **Follow-up consistency:** if a follow-up asks about the same series
  ...
  earlier turns: carry over the settled series, and horizon and
