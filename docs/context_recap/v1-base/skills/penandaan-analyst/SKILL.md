...
- TOTAL SQL ceiling per turn: **4**. Reaching the ceiling without a defensible number = STOP and
...
- Free-text product search (nama_produk): try a coded column first; only then ONE combined query
  `(nama_produk ILIKE '%kw1%' OR produsen ILIKE '%kw1%') AND (... kw2 ...)` with a LIMIT, max 2 probes total
...
- On a follow-up, read the earlier turns first: carry over the settled subject, komoditi/scope,
  time range, and resolved codes, and change only what this turn names — never rebuild from a blank
...
- **The code set is closed.** Compound concepts — "X atau Y", "memenuhi", "tidak sesuai" —
  take every member from the §4 closure table or from a full category read.
  Never the first single-keyword ILIKE hit: the sibling it drops is invisible in the result
  (`filter_code_reference.md` §0, §10).
- **Headline total came from its OWN global DISTINCT query**, never summed from a partitioned
  breakdown — per-year, per-komoditi and per-balai rows double-count revisions (`predikat.md` §10).
- **Kesimpulan filter = the asked population.** Never stack MK on a population that is
  already defined by another kesimpulan; that erases what was asked about.
- **Exclusions applied** — unnamed balai, ed_nie outliers, empty nomorsampel.
...
- **Casing normalisation applied** if kesimpulan_balai was used (`predikat.md` §6).
...
answer ("apa itu penandaan") skips the export entirely — counts against the budget the same as any
other tool call.
...
Shape the answer per the Answer Contract (`predikat.md` §10): canonical interpretation first,
