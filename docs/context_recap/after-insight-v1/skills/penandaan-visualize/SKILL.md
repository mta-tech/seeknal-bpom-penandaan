...
tags: [bpom, penandaan, chart, visualization]
version: "1.0.0"
---
...
Load `context/predikat.md` + `context/filter_code_reference.md` before building chart SQL,
...
| comparison across categories (komoditi, balai, kesimpulan) | `bar_chart` |
| the same categories split by a second dimension | `grouped_bar_chart` |
| categories with long labels (balai names) | `horizontal_bar_chart` |
| share of one whole, few slices (kesimpulan distribution) | `pie_chart` |
| a value across two dimensions at once (komoditi × kesimpulan) | `heatmap` |
| two quantitative columns compared | `scatter_plot` |
| spread within categories (duration distribution) | `box_plot` |
| nested part-of-whole | `treemap` |
| a single scalar (total penandaan) | `big_number` |
...
When the answer covers several komoditi, balai, or kesimpulan codes, the chart shows
...
```sql
SELECT periode, 'MK' AS seri, mk AS jumlah FROM t
UNION ALL
SELECT periode, 'TMK' AS seri, tmk AS jumlah FROM t
UNION ALL
SELECT periode, 'VP' AS seri, vp AS jumlah FROM t
ORDER BY 1, 2
```
...
query — a `GROUP BY periode, kesimpulan` — and the wide table is only how it was
...
- **One chart per question — but one chart carries many series.** "One chart"
  ...
  showing four komoditi is one chart. Choose the most informative chart, then make
...
- **Chart follows the answer's filters.** Same counting entity, same kesimpulan and
  komoditi codes, same date range. If you cannot reuse the answer's rows, do
  not chart.
- **The chart is not the export.** A chart never replaces `upload_to_s3`; the CSV
  store contract stays as in `penandaan-analyst/SKILL.md`.
